...
def _redline_tasks_amp() -> List[Tuple[str, float, float]]:
    try:
        from amp_taskset import REDLINE_TASKS_AMP  # type: ignore

        out: List[Tuple[str, float, float]] = []
        for t in list(REDLINE_TASKS_AMP):
            try:
                out.append((str(t.family).strip(), float(t.vin), float(t.vout)))
            except Exception:
                continue
        if out:
            return out
        ...
        pass
    # Fallback: full taskset (10 targets).
    return [
        ("amp_op2", 6.0, 1e4),
        ("amp_op2", 6.0, 3e4),
        ("amp_op2", 6.0, 1e5),
...
REDLINE_TASKS: List[Tuple[str, float, float]] = _redline_tasks_amp()
...
def _maybe_accelerate(cmd: List[str], *, nproc: int) -> List[str]:
    ...
    if exe:
        ...
        # Some environments ship the Python package without installing the CLI entrypoint.
        # Use module invocation as a robust fallback.
        launcher = [sys.executable, "-m", "accelerate.commands.launch"]
    return [
        *launcher,
        "--multi_gpu",
        "--num_processes",
...
def _run_redline_eval(
    *,
    ...
    # Amp redline eval uses a fixed guard set (amp_taskset.REDLINE_TASKS_AMP).
    outdir.mkdir(parents=True, exist_ok=True)
    cmd = [
        sys.executable,
        str(code_dir / "eval_amp_family.py"),
        "--base_model",
        str(base_model),
        "--adapter",
        str(adapter),
...
def main() -> int:
    ...
    ap.add_argument("--run_root", default="/root/autodl-tmp/vpspi_multitype/runs")
    ...
    ap.add_argument("--selfplay_min_pass_cv", type=int, default=1)
    ...
    ap.add_argument("--selfplay_sim_workers", type=int, default=0, help="Parallel ngspice workers for self-play. 0=auto(use cgroup CPU quota).")
    ap.add_argument(
        "--selfplay_shards",
        type=int,
        default=0,
    ...
    ap.add_argument("--selfplay_repair_factors", default="0.05,0.1,0.2,0.5,0.8,1.0,1.25,1.5,2.0,3.0,5.0,10.0")
    ap.add_argument("--selfplay_repair_max_evals", type=int, default=60)
    ap.add_argument("--selfplay_repair_max_iters", type=int, default=6)
    ...
    # (A') Repair-distill: teach the policy to match the EDA repair outcome (boost open-loop).
    ap.add_argument("--repair_distill_enable", action=argparse.BooleanOptionalAction, default=True)
    ap.add_argument("--repair_distill_require_after_pass_cv", action=argparse.BooleanOptionalAction, default=True)
    ap.add_argument("--repair_distill_require_improvement", action=argparse.BooleanOptionalAction, default=False)
    ap.add_argument("--repair_distill_min_score_gain", type=float, default=0.0)
    ap.add_argument("--repair_distill_max_pairs_per_task", type=int, default=0)
    ap.add_argument("--repair_distill_max_pairs", type=int, default=0)
    ap.add_argument("--repair_distill_min_pairs", type=int, default=16)
    ap.add_argument("--repair_distill_mix_into_dpo", action=argparse.BooleanOptionalAction, default=True)
    ap.add_argument("--repair_distill_mix_into_sft_warmup", action=argparse.BooleanOptionalAction, default=True)
    ap.add_argument("--repair_distill_sft_aug_per_record", type=int, default=5)
    ap.add_argument("--repair_distill_sft_aug_seed", type=int, default=2025)
    ap.add_argument("--repair_distill_sft_aug_keep_names", default="RBIAS,RFB,RG,CCOMP")
    ...
    ap.add_argument("--ppo_sim_timeout_s", type=float, default=60.0)
    ap.add_argument("--ppo_sim_workers", type=int, default=0, help="Parallel ngspice workers for PPO rollout/eval. 0=auto(use cgroup CPU quota).")
    ap.add_argument("--ppo_save_steps", type=int, default=10)
    ap.add_argument(
        "--ppo_skip_if_init_cv_below",
        type=float,
        default=0.05,
        help="Skip Safe-PPO for this round if the PPO init adapter has redline cv_rate below this value (format gate; avoids wasting steps when open-loop is broken).",
    )
    ap.add_argument(
        "--ppo_guard_schedule",
    ...
    if bool(args.resume):
        ...
        if fin.exists():
            try:
                ...
                pass
        if not str(cur_adapter or '').strip() or str(cur_adapter).strip() == str(args.anchor_adapter).strip():
            try:
                ...
                for p in states:
                    try:
                        ...
                        rr = int(obj.get('round'))
                        if (best_round is None) or (rr > best_round):
                            best_round = rr
                            best_path = p
                    except Exception:
                        continue
                if best_path is not None:
                    obj = json.loads(best_path.read_text(encoding='utf-8', errors='ignore'))
                    cur2 = str(obj.get('cur_adapter') or '').strip()
                    if cur2:
                        cur_adapter = cur2
                        _append_text(log_path, f"[resume] cur_adapter <- round_state.json ({cur_adapter})\n")
            except Exception:
                pass
    ...
    while True:
        ...
        base_sp_cmd = [
            sys.executable,
            str(code_dir / "build_selfplay_amp_datasets.py"),
            "--base_model",
            str(args.base_model),
            "--adapter",
            str(cur_adapter),
        ...
        if bool(args.selfplay_eda_repair):
            ...
            if bool(args.selfplay_no_structural_repair):
                base_sp_cmd.append("--no_structural_repair")
        ...
        # Build repair-distill datasets from the per-task scored records (no GPU required).
        if bool(args.repair_distill_enable):
            distill_cmd = [
                sys.executable,
                str(code_dir / "build_repair_distill_from_selfplay.py"),
                "--selfplay_root",
                str(sp_dir),
                "--min_score_gain",
                str(float(args.repair_distill_min_score_gain)),
                "--max_pairs_per_task",
                str(int(args.repair_distill_max_pairs_per_task)),
                "--max_pairs",
                str(int(args.repair_distill_max_pairs)),
                "--sft_aug_per_record",
                str(int(args.repair_distill_sft_aug_per_record)),
                "--sft_aug_seed",
                str(int(args.repair_distill_sft_aug_seed)),
                "--sft_aug_keep_names",
                str(args.repair_distill_sft_aug_keep_names),
            ]
            if bool(args.repair_distill_require_after_pass_cv):
                distill_cmd.append("--require_after_pass_cv")
            if bool(args.repair_distill_require_improvement):
                distill_cmd.append("--require_improvement")
            if bool(args.resume):
                distill_cmd.append("--resume")
            _run(distill_cmd, cwd=code_dir, log_path=log_path)
        ...
        if int(getattr(args, "selfplay_sft_warmup_steps", 0) or 0) > 0:
            ...
            if strict_path.exists() and int(strict_path.stat().st_size) > 0:
                warm_train_jsonl = strict_path
                if bool(args.repair_distill_mix_into_sft_warmup):
                    distill_sft = sp_dir / "repair_distill_sft_train.jsonl"
                    if distill_sft.exists() and int(distill_sft.stat().st_size) > 0:
                        warm_mix = rdir / "selfplay_sft_warmup_mix.jsonl"
                        if (not bool(args.resume)) or (not warm_mix.exists()) or warm_mix.stat().st_size <= 0:
                            n_mix = _concat_jsonl(warm_mix, strict_path, distill_sft)
                            _append_text(log_path, f"[warmup_mix] wrote {n_mix} rows -> {warm_mix}\n")
                        warm_train_jsonl = warm_mix
        ...
        base_pairs_path = sp_dir / "dpo_pairs.jsonl"
        pairs_path = base_pairs_path
        distill_pairs = sp_dir / "repair_distill_dpo_pairs.jsonl"
        if bool(args.repair_distill_mix_into_dpo) and distill_pairs.exists() and distill_pairs.stat().st_size > 0:
            n_distill = _count_lines(distill_pairs)
            if n_distill >= int(args.repair_distill_min_pairs):
                mix_path = sp_dir / "dpo_pairs_with_repair_distill.jsonl"
                if (not bool(args.resume)) or (not mix_path.exists()) or mix_path.stat().st_size <= 0:
                    n_mix = _concat_jsonl(mix_path, base_pairs_path, distill_pairs)
                    _append_text(log_path, f"[pvpo_pairs_mix] base={base_pairs_path} distill={distill_pairs} -> {mix_path} rows={n_mix}\n")
                pairs_path = mix_path

        if (not pairs_path.exists()) or int(pairs_path.stat().st_size) <= 0:
            _append_text(log_path, f"[pvpo_skip] missing/empty pairs: {pairs_path}\n")
            _write_json(
                rdir / "pvpo_gate.json",
                {
                    "accepted": False,
                    "skipped": True,
                    "reason": "missing_or_empty_pairs",
            ...
            if bool(args.resume) and dpo_adapter.exists():
                _append_text(log_path, '[pvpo] reuse existing dpo_final: ' + str(dpo_adapter) + '\n')
            else:
                if bool(args.resume):
                    dpo_cmd.append('--resume')
            ...
            if str(args.pvpo_sft_reg_jsonl or "").strip():
                ...
                if bool(args.resume) and reg_adapter.exists():
                    _append_text(log_path, '[pvpo] reuse existing sft_final: ' + str(reg_adapter) + '\n')
                else:
                    if bool(args.resume):
                        reg_cmd.append('--resume')
        ...
        # If the PPO init adapter has essentially zero open-loop CV on the redline guard,
        # running PPO tends to produce near-constant failure rewards and wastes compute.
        # This gate makes the pipeline resume-safe and avoids "learning on garbage".
        try:
            init_cv = float(dpo_eval.get("cv_rate") or 0.0) if bool(ok_dpo) else float(best_eval.get("cv_rate") or 0.0)
        except Exception:
            init_cv = 0.0
        if float(getattr(args, "ppo_skip_if_init_cv_below", 0.0) or 0.0) > 0.0 and init_cv < float(args.ppo_skip_if_init_cv_below):
            _append_text(
                log_path,
                f"[ppo] skipped: init_cv={init_cv:.4f} < ppo_skip_if_init_cv_below={float(args.ppo_skip_if_init_cv_below):.4f} (ridx={ridx})\n",
            )
            _write_json(
                rdir / "ppo_skipped.json",
                {
                    "round": int(ridx),
                    "init_cv": float(init_cv),
                    "threshold": float(args.ppo_skip_if_init_cv_below),
                    "cur_adapter": str(cur_adapter),
                    "cur_after_pvpo": str(cur_after_pvpo),
                },
            )
            ...
            _write_json(outdir / "final_state.json", {"cur_adapter": str(cur_adapter), "best_eval": best_eval, "last_round": int(ridx)})
            ridx += 1
            continue
        ...
        # Avoid duplicate eval: Safe-PPO already selects ppo_best by CV-guard; reuse its stored guard metrics.
        ppo_redline_dir = rdir / "ppo_redline"
        ppo_redline_dir.mkdir(parents=True, exist_ok=True)
        ppo_eval: Optional[Dict[str, Any]] = None
        try:
            meta_path = ppo_best / "best_meta.json"
            if meta_path.exists():
                meta = json.loads(meta_path.read_text(encoding="utf-8"))
                cand = meta.get("cv_guard")
                if isinstance(cand, dict) and cand:
                    ppo_eval = cand
                    _write_json(ppo_redline_dir / "redline_metrics.json", ppo_eval)
        except Exception:
            ppo_eval = None
