...
from amp_eval_acop import AMP_BW_TOL_REL, AMP_GAIN_TOL_DB, amp_family_params, eval_one_detail_amp_family
from dcdc_taskset import Task
from amp_taskset import REDLINE_TASKS_AMP, default_taskset_amp
...
def _guard_tasks_default() -> List[Tuple[str, float, float]]:
    return [(str(t.family).lower(), float(t.vin), float(t.vout)) for t in list(REDLINE_TASKS_AMP)]
    ...
    *,
    ...
    for fam, vin, vout in tasks:
        ...
        for i, r in enumerate(resp):
            ...
            ver = verify_inc_amp(inc, min_elems=int(min_elems))
...
def compute_reward(
    ...
    *,
    ...
    # Element-count shaping: keep >=min_elems as the strict success criterion,
    # but provide a graded penalty so PPO can learn before it reaches 20+ elements.
    size_frac = 1.0
    if int(min_elems) > 0:
        size_frac = max(0.0, min(1.0, float(n_elems) / float(min_elems)))
    pen_size = -1.0 * (1.0 - size_frac)

    # Rule-level shaped reward to prevent collapse to empty/invalid outputs.
    viol_set = set(str(x) for x in (violations or []))

    def _has(prefix: str) -> bool:
        return any(v.startswith(prefix) for v in viol_set)

    checks: List[Tuple[str, float]] = [
        ("missing_node_vin", 1.0),
        ("missing_node_inv", 1.0),
        ("missing_node_out", 1.0),
        ("missing_node_vdd", 1.0),
        ("missing_node_0", 1.0),
        ("numeric_node_not_allowed", 2.0),
        ("vin_must_only_connect_to_0", 2.0),
        ("missing_bias_R_vdd_0", 2.0),
        ("missing_feedback_R_out_inv", 2.0),
        ("missing_feedback_R_inv_0", 2.0),
        ("nonessential_R_must_be_open", 1.0),
        ("nonessential_C_must_be_tiny", 1.0),
        ("nonessential_L_must_be_large", 1.0),
        ("min_elems", 2.0),
        ("amp_disallowed_kinds_", 2.0),
    ]
    ...
    if int(n_elems) <= 0:
        ...
        reward_rule = -1.0 + 0.8 * float(rule_score)
    ...
    pm_min = float(params.get("pm_min_deg") or 0.0)
    p_max = float(params.get("pstatic_max_mw") or 1.0)
    ...
    try:
        ...
        g, bw, pm, p = 0.0, 0.0, 0.0, float(p_max)

    gain_err = abs(float(g) - float(gain_db_t))
    bw_err = abs(float(bw) - float(bw_hz_t)) / max(1e-9, float(bw_hz_t))

    # Use *shaping* scales decoupled from strict pass/fail tolerances.
    gain_shape_db = float(max(float(AMP_GAIN_TOL_DB), 6.0))
    bw_shape_decades = 0.50  # half-decade: provides useful gradients even when far from target
    gain_n = float(gain_err / max(1e-6, float(gain_shape_db)))
    try:
        bw_ratio = float(max(1e-12, float(bw)) / max(1e-12, float(bw_hz_t)))
        bw_log_err = abs(float(math.log10(bw_ratio)))
    except Exception:
        bw_log_err = 10.0
    bw_n = float(bw_log_err / max(1e-6, float(bw_shape_decades)))
    pm_n = max(0.0, float(pm_min - float(pm)) / max(1e-6, float(pm_min)))

    worst = max(gain_n, bw_n, pm_n)
    pass_cv = bool(detail.get("pass_CV", False))

    if pass_cv:
        margin = min(1.0, max(0.0, float(worst)))
        cv_term = 1.0 - 0.20 * margin
    else:
        over = max(0.0, float(worst) - 1.0)
        k = 2.0
        cv_term = -float(over) / float(over + k)

    reward_cv = float(weights.w_cv * float(cv_term) * float(size_frac))

    # Only optimize power once the main specs are satisfied.
    if not pass_cv:
        reward_ce = 0.0
    else:
        p_margin = (float(p_max) - float(p)) / max(1e-6, float(p_max))
        p_term = max(-1.0, min(1.0, float(p_margin)))
        reward_ce = float(weights.w_ce * float(p_term) * float(size_frac))

    total = float(reward_cv + reward_ce + pen_size)
    parts = {
        "reward_total": total,
        "reward_rule": 0.0,
        "rule_score": float(rule_score),
        "reward_cv": float(reward_cv),
        "reward_ce": float(reward_ce),
        "pen_ripple": 0.0,
        "pen_over": 0.0,
        "pen_size": float(pen_size),
        "pen_fail": 0.0,
    ...
    *,
    ...
    for _, idxs in by_group.items():
        ...
        for i in succ:
            rec = details[i] or {}
            d = rec.get("detail") or {}
            gain_t = float(rec.get("vin") or 0.0)
            bw_t = float(rec.get("vout") or 0.0)
            gain = float(d.get("gain_db") or 0.0)
            bw = float(d.get("bw_hz") or 0.0)
            pm = float(d.get("pm_deg") or 0.0)
            p = float(d.get("pstatic_mw") or 0.0)
            n_elems = float(rec.get("n_elems") or 0.0)

            params = amp_family_params(str(rec.get("family") or ""))
            pm_min = float(params.get("pm_min_deg") or 0.0)
            p_max = float(params.get("pstatic_max_mw") or 1.0)

            gain_err = abs(float(gain) - float(gain_t)) / max(1e-6, float(AMP_GAIN_TOL_DB))
            bw_err = abs(float(bw) - float(bw_t)) / max(1e-9, float(bw_t)) / max(1e-6, float(AMP_BW_TOL_REL))
            pm_n = max(0.0, float(pm_min - float(pm)) / max(1e-6, float(pm_min)))
            p_n = max(0.0, (float(p) - float(p_max)) / max(1e-6, float(p_max)))

            # Minimize: gain_err, bw_err, pm_violation, power_violation; Maximize: n_elems
            objs.append((gain_err, bw_err, pm_n, p_n, -n_elems))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "'pareto_add' adds a Pareto-GRPO-style relative term based on (gain_err, bw_err, pm_violation, power_violation, n_elems)."
        ),
    )
    ap.add_argument(
        "--group_rel_coef",
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "(e.g., amp_rfpa) from dominating gradients."
        ),
    )
    ap.add_argument(
        "--train_task_pool",
    ...
    from eval_amp_family import build_prompt
    ...
    for step in range(int(start_step), int(args.steps)):
        ...
        for bi, (t, resp_ids) in enumerate(zip(batch_tasks, responses)):
            ...
            ver = verify_inc_amp(inc, min_elems=int(args.min_elems))
            ...
            if not bool(ver.ok):
                ...
                if sim_executor is None:
                    info["detail"] = eval_one_detail_amp_family(
                        inc,
                        family=str(t.family),
                        gain_db=float(t.vin),
                        bw_hz=float(t.vout),
        ...
        for bi, info in enumerate(sample_infos):
            ...
            reward_total, parts = compute_reward(
                detail,
                family=str(t.family),
                gain_db_t=float(t.vin),
                bw_hz_t=float(t.vout),
                weights=weights,
                min_elems=int(args.min_elems),
                n_elems=n_elems,
        ...
        print(
            ...
            f"ok={ok_rate:.3f} cv={cv_rate:.3f} ce={ce_rate:.3f} min{int(args.min_elems)}={min_elems_rate:.3f} "
