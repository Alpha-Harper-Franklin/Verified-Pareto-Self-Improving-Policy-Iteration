...
from amp_eval_acop import eval_one_detail_amp_family
...
def build_prompt(family: str, vin: float, vout: float, *, min_elems: int) -> str:
    from eval_amp_family import build_prompt as _bp  # type: ignore
...
def _eval_one_amp(task: Task, raw: str, *, sim_timeout_s: float, min_elems: int) -> Dict[str, Any]:
    return eval_one_detail_amp_family(
        raw,
        family=str(task.family),
        gain_db=float(task.vin),
        bw_hz=float(task.vout),
        sim_timeout_s=float(sim_timeout_s),
        min_elems=int(min_elems),
    )


def _maybe_repair_amp(
    task: Task,
    raw: str,
    *,
    sim_timeout_s: float,
    ...
    from build_selfplay_amp_datasets import _eda_repair  # type: ignore

    base_detail = _eval_one_amp(task, raw, sim_timeout_s=sim_timeout_s, min_elems=min_elems)
    ...
    inc2, det2, meta = _eda_repair(
        raw,
        family=str(task.family),
        gain_db_t=float(task.vin),
        bw_hz_t=float(task.vout),
        sim_timeout_s=float(sim_timeout_s),
        min_elems=int(min_elems),
        factors=list(repair_factors),
        max_evals=int(repair_max_evals),
        max_iters=int(repair_max_iters),
        always=False,
        no_structural_repair=bool(no_structural_repair),
    )
    meta = dict(meta or {})
...
def main() -> int:
    ...
    ap.add_argument(
        "--repair_only_on_fail",
        action="store_true",
        help="If set: repair only when ok==False (sim fail). Default repairs any non-pass_CV.",
    )
    ap.add_argument("--repair_factors", default="0.05,0.1,0.2,0.5,0.8,1.0,1.25,1.5,2.0,3.0,5.0,10.0")
    ap.add_argument("--repair_max_evals", type=int, default=60)
    ...
    for ti, t in enumerate(tasks):
        ...
        task_key = f"{t.family.lower()}/vin{float(t.vin):.1f}_vout{float(t.vout):.6g}"
        task_dir = outdir / "tasks" / t.family.lower() / f"vin{float(t.vin):.1f}_vout{float(t.vout):.6g}"
    ...
    def _job_open(j: Tuple[str, SampleRec]) -> Tuple[str, Dict[str, Any]]:
        key, rec = j
        d = _eval_one_amp(rec.task, rec.raw, sim_timeout_s=float(args.sim_timeout_s), min_elems=int(args.min_elems))
    ...
    if bool(args.with_repair):
        ...
        def _job_rep(item: Tuple[str, Dict[str, Any]]) -> Tuple[str, Dict[str, Any]]:
            ...
            inc2, det2, meta = _maybe_repair_amp(
                t,
                str(rec["raw"]),
