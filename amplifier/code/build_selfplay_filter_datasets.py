...
def _build_prompt(family: str, vin: float, vout: float, *, min_elems: int) -> str:
    from eval_filter_family import build_prompt  # type: ignore
...
def _score_filter(detail: Dict[str, Any], *, family: str, vin: float, vout: float) -> float:
    ...
    # Add a smooth error term to help ranking among failures.
    try:
        if fam in {"filter_lpf", "filter_hpf"}:
            fc_t = float(vin)
            fc = float(detail.get("fc_est") or 0.0)
            err = abs(fc - fc_t) / max(1e-9, fc_t)
            score += 2.0 * max(-1.0, 1.0 - err / max(1e-6, float(FILTER_F_TOL_REL)))
        elif fam == "filter_bpf":
            f0_t = float(vin)
            bw_t = float(vout)
            ...
            score += 1.5 * max(-1.0, 1.0 - err0 / max(1e-6, float(FILTER_F_TOL_REL)))
            score += 0.5 * max(-1.0, 1.0 - errb / max(1e-6, float(2.0 * FILTER_F_TOL_REL)))
        elif fam == "filter_notch":
            f0_t = float(vin)
            f0 = float(detail.get("f0_est") or 0.0)
            err0 = abs(f0 - f0_t) / max(1e-9, f0_t)
            score += 2.0 * max(-1.0, 1.0 - err0 / max(1e-6, float(FILTER_F_TOL_REL)))
    except Exception:
        pass
...
def _eval_one(
    ...
    *,
    ...
    score = _score_filter(detail, family=str(family), vin=float(vin), vout=float(vout))
...
def main() -> int:
    ...
    ap.add_argument("--min_pass_cv", type=int, default=2)
    ...
    # Load model (one process -> one GPU; sharding handled by the pipeline).
    tok, model = _load_model(str(args.base_model), str(args.adapter or ""))
    dev = _device(model)

    tasks_all = default_taskset_filter()
    tasks: List[Task] = []
    ...
    # Open output files (append-only for resume).
        ...
        for ti, task in enumerate(tasks):
            ...
            for ridx in range(int(args.max_rounds)):
                ...
                def _eval_candidate(inc0: str) -> Tuple[Dict[str, Any], str]:
                    ...
                    score = _score_filter(detail, family=fam, vin=vin, vout=vout)
                    dt = float(time.time() - t0)
            ...
            if not chosen_pool:
                # still write done; PPO can learn from verifier reward, but PVPO needs CV-passing chosen.
                chosen_pool = []
