...
from filter_eval_ac import FILTER_F_TOL_REL, eval_one_detail_filter
from dcdc_taskset import Task
from filter_taskset import REDLINE_TASKS_FILTER, default_taskset_filter
...
def _guard_tasks_default() -> List[Tuple[str, float, float]]:
    return [(str(t.family).lower(), float(t.vin), float(t.vout)) for t in list(REDLINE_TASKS_FILTER)]
    ...
    *,
    ...
    for fam, vin, vout in tasks:
        ...
        for i, r in enumerate(resp):
            ...
            ver = verify_inc_filter(inc, min_elems=int(min_elems))
...
def compute_reward(
    ...
    *,
    ...
    checks: List[Tuple[str, float]] = [
        ...
        ("node_not_allowed", 2.0),
        ("missing_reactive", 2.0),
        ("disconnected_vin_to_out", 2.0),
        ("min_elems", 2.0),
        ("filter_disallowed_kinds_", 2.0),
    ]
    ...
    # Normalized spec error (for shaped reward among failures).
    worst = 2.0
    try:
        if fam in {"filter_lpf", "filter_hpf"}:
            fc = float(detail.get("fc_est") or 0.0)
            fc_t = float(vin_t)
            err = abs(fc - fc_t) / max(1e-9, fc_t)
            worst = float(err) / max(1e-6, float(FILTER_F_TOL_REL))
        elif fam == "filter_bpf":
            f0 = float(detail.get("f0_est") or 0.0)
            bw = float(detail.get("bw_est") or 0.0)
            f0_t = float(vin_t)
            bw_t = float(vout_t)
            err0 = abs(f0 - f0_t) / max(1e-9, f0_t)
            errb = abs(bw - bw_t) / max(1e-9, bw_t)
            worst = max(float(err0) / max(1e-6, float(FILTER_F_TOL_REL)), float(errb) / max(1e-6, float(2.0 * FILTER_F_TOL_REL)))
        elif fam == "filter_notch":
            f0 = float(detail.get("f0_est") or 0.0)
            f0_t = float(vin_t)
            err0 = abs(f0 - f0_t) / max(1e-9, f0_t)
            worst = float(err0) / max(1e-6, float(FILTER_F_TOL_REL))
    except Exception:
        worst = 2.0
    ...
    if not pass_cv:
        ...
        ce_term = 1.0 if pass_ce else -1.0
        reward_ce = float(weights.w_ce * float(ce_term) * float(size_frac))
    ...
    *,
    ...
    for _, idxs in by_group.items():
        ...
        for i in succ:
            ...
            vin_t = float(rec.get("vin") or 0.0)
            vout_t = float(rec.get("vout") or 0.0)
            fc = float(d.get("fc_est") or 0.0)
            f0 = float(d.get("f0_est") or 0.0)
            bw = float(d.get("bw_est") or 0.0)
            il0 = float(d.get("g0_db") or 0.0)
            n_elems = float(rec.get("n_elems") or 0.0)

            if fam in {"filter_lpf", "filter_hpf"}:
                err = abs(fc - vin_t) / max(1e-9, vin_t)
            elif fam == "filter_bpf":
                err0 = abs(f0 - vin_t) / max(1e-9, vin_t)
                errb = abs(bw - vout_t) / max(1e-9, vout_t)
                err = max(err0 / max(1e-6, float(FILTER_F_TOL_REL)), errb / max(1e-6, float(2.0 * FILTER_F_TOL_REL)))
            else:
                err = abs(f0 - vin_t) / max(1e-9, vin_t)

            # Minimize: spec_err, insertion_loss; Maximize: n_elems
            objs.append((float(err), float(-il0), float(-n_elems)))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "'pareto_add' adds a Pareto-GRPO-style relative term based on (spec_err, insertion_loss, n_elems)."
        ),
    )
    ap.add_argument(
        "--group_rel_coef",
    ...
    from eval_filter_family import build_prompt
    ...
    for step in range(int(start_step), int(args.steps)):
        ...
        for bi, (t, resp_ids) in enumerate(zip(batch_tasks, responses)):
            ...
            ver = verify_inc_filter(inc, min_elems=int(args.min_elems))
            ...
            if not bool(ver.ok):
                ...
                if sim_executor is None:
                    info["detail"] = eval_one_detail_filter(
                        inc,
                        family=str(t.family),
                        vin=float(t.vin),
                        vout=float(t.vout),
