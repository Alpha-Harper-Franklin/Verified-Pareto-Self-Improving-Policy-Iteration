...
from osc_taskset import REDLINE_TASKS_OSC, default_taskset_osc
...
def _sanity_generate_report(
    *,
    ...
    tasks = [("osc_rc", 1e4, 1.0), ("osc_lc", 1e6, 1.0)]
...
def _guard_tasks_default() -> List[Tuple[str, float, float]]:
    return [(str(t.family).lower(), float(t.vin), float(t.vout)) for t in list(REDLINE_TASKS_OSC)]
    ...
    *,
    ...
    for fam, vin, vout in tasks:
        ...
        for i, r in enumerate(resp):
            ...
            ver = verify_inc_osc(inc, min_elems=int(min_elems))
...
def compute_reward(
    ...
    *,
    ...
    try:
        ...
        f_t = float(vin_t)
        f_err = abs(float(f) - float(f_t)) / max(1e-9, float(f_t))
        f_n = float(f_err) / max(1e-6, float(OSC_F_TOL_REL))

        vpp = float(detail.get("vpp") or 0.0)
        v_def = max(0.0, (float(OSC_MIN_VPP) - float(vpp)) / max(1e-6, float(OSC_MIN_VPP)))

        worst = max(float(f_n), float(v_def))
    except Exception:
        worst = 2.0
    ...
    *,
    ...
    for _, idxs in by_group.items():
        ...
        for i in succ:
            ...
            f = float(d.get("freq_hz") or 0.0)
            vpp = float(d.get("vpp") or 0.0)
            p = float(d.get("pstatic_mw") or 0.0)
            n_elems = float(rec.get("n_elems") or 0.0)

            f_err = abs(float(f) - float(vin_t)) / max(1e-9, float(vin_t))
            err = float(f_err) / max(1e-6, float(OSC_F_TOL_REL))

            # Minimize: spec_err, power; Maximize: vpp, n_elems
            objs.append((float(err), float(p), float(-vpp), float(-n_elems)))
...
def main() -> None:
    ...
    from eval_osc_family import build_prompt
    ...
    for step in range(int(start_step), int(args.steps)):
        ...
        for bi, (t, resp_ids) in enumerate(zip(batch_tasks, responses)):
            ...
            ver = verify_inc_osc(inc, min_elems=int(args.min_elems))
            ...
            if not bool(ver.ok):
                ...
                if sim_executor is None:
                    info["detail"] = eval_one_detail_osc(
                        inc,
                        family=str(t.family),
                        vin=float(t.vin),
                        vout=float(t.vout),
