...
def _redline_tasks_osc() -> List[Tuple[str, float, float]]:
    try:
        from osc_taskset import REDLINE_TASKS_OSC  # type: ignore

        out: List[Tuple[str, float, float]] = []
        for t in list(REDLINE_TASKS_OSC):
            try:
        ...
        pass
    # Fallback: full taskset (20 targets).
    out: List[Tuple[str, float, float]] = []
    for fam, freqs in [
        ("osc_lc", [1e4, 2e4, 5e4, 1e5, 2e5]),
        ("osc_rc", [1e3, 2e3, 5e3, 1e4, 2e4]),
        ("osc_ring", [1e5, 2e5, 5e5, 1e6, 2e6]),
        ("osc_wien", [1e3, 2e3, 5e3, 1e4, 2e4]),
    ]:
        for f in freqs:
            out.append((fam, float(f), 1.0))
    return out


REDLINE_TASKS: List[Tuple[str, float, float]] = _redline_tasks_osc()
...
def _run_redline_eval(
    *,
    ...
    cmd = [
        sys.executable,
        str(code_dir / "eval_osc_family.py"),
        "--base_model",
        str(base_model),
        "--adapter",
        str(adapter),
...
def main() -> int:
    ...
    while True:
        ...
        base_sp_cmd = [
            sys.executable,
            str(code_dir / "build_selfplay_osc_datasets.py"),
            "--base_model",
            str(args.base_model),
            "--adapter",
            str(cur_adapter),
